...
**Migrations are like version control for your database.** Instead of telling a teammate "run this SQL by hand", you commit PHP files that describe schema changes — then everyone (and every server) builds the identical schema with one Artisan command. The `Schema` facade provides **database-agnostic** table creation across all supported databases (SQLite, MySQL/MariaDB, PostgreSQL, SQL Server).

Key pieces of every migration:

| Piece | Purpose |
| ----- | ------- |
| `php artisan make:migration create_shops_table` | Generates the file in `database/migrations`. Laravel parses the name — `create_*_table` pre-fills `Schema::create('shops', ...)` |
| Filename timestamp `2026_09_16_161759` | The execution order — migrations run **oldest first** |
| Anonymous class `return new class extends Migration` | Modern migration style — a throwaway class defined right in the file |
| `up()` | **Applies** the schema change (create table / add column / ...) |
| `down()` | **Reverses** what `up()` did — what makes a migration rollback-able |
| `migrations` table | Laravel's bookkeeping — each executed migration is stored with its **batch** number |

The command toolbox (from the docs):

| Command | What it does |
| ------- | ------------ |
| `php artisan migrate` | Runs all **outstanding** (pending) migrations |
| `php artisan migrate:status` | Shows which migrations ran and which are pending |
| `php artisan migrate:rollback` | Rolls back the **last batch** (`--step=N` for N migrations, `--batch=3` for a specific batch) |
| `php artisan migrate:reset` | Rolls back **all** migrations |
| `php artisan migrate:refresh` | Rollback + migrate — rebuilds the schema via your `down()`s (`--seed` to re-seed) |
| `php artisan migrate:fresh` | **Drops all tables**, then migrates — never calls your `down()`s |

> **⚠️ Heads-up:** `migrate:refresh` still executes each `down()`, but `migrate:fresh` simply drops every table — your `down()` methods never run. Also, in production Laravel prompts before running destructive commands; `--force` skips the prompt.

Column methods & modifiers used in this lesson:

| Line | What it creates |
| ---- | --------------- |
| `$table->id()` | Auto-incrementing primary key — an alias of `bigIncrements('id')` (`UNSIGNED BIGINT`) |
| `$table->string('shop_name')` | `VARCHAR(255)` (pass `length:` for a different size) |
| `$table->integer('shop_number')` | `INTEGER` |
| `$table->timestamps()` | The `created_at` + `updated_at` timestamp pair |
| `->nullable()` | Allows `NULL` values in that column |
| `->comment('Shop Name')` | A **column** comment — MariaDB / MySQL / PostgreSQL only |
| `$table->comment('Shop Table')` | A **table** comment — also MariaDB / MySQL / PostgreSQL only |
...
```php
// Modify an existing table instead of creating one
Schema::table('shops', function (Blueprint $table) {
    $table->string('owner')->nullable();   // add a column
    $table->renameColumn('owner', 'boss'); // rename it
    $table->dropColumn('boss');            // or drop it
});

// Indexes
$table->string('shop_email')->unique();   // unique index
$table->index(['shop_name', 'shop_number']); // compound index

// Foreign keys — the short way
...
The resulting table (verified on SQLite — no comments, since SQLite ignores them):

```sql
CREATE TABLE "shops" (
    "id" integer primary key autoincrement not null,
    "shop_name" varchar, "shop_number" integer,
    "shop_address" varchar, "shop_phone" varchar, "shop_email" varchar,
...
```mermaid
flowchart TD
    A["php artisan migrate"] --> B["Read database/migrations<br/>sorted by filename timestamp"]
    B --> C{"Already recorded in the<br/>migrations table?"}
    C -- "yes" --> D["Skip — already applied"]
    C -- "no" --> E["Run up()<br/>Schema::create builds the shops table"]
    E --> F["Record filename + batch number<br/>in the migrations table"]
    F --> G["php artisan migrate:rollback<br/>→ runs down() for the last batch"]
    G --> H["dropIfExists('shops')<br/>table removed, batch row deleted"]
```
